...
echo -e "CREATE TABLE #DBCCOut (name varchar(256), size decimal(15,8), used decimal(15,8), status int)\nINSERT INTO #DBCCOut EXEC ('DBCC SQLPERF(LOGSPACE)')\ngo\nSELECT name, used from #DBCCOut where used > $pct_thresh\ngo" > $tmpfile
