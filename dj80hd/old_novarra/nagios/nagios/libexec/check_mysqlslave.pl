...
## Added: groundwork specific libs, Fri Jan 13 16:09:09 PST 2006, vnovoselskiy
...
if ($opt_V) {
	print_revision($PROGNAME,'$Revision: 1.1.1.1 $REVISION' .' $' );
	exit $ERRORS{'OK'};
}
...
# Uncomment the following lines to see what $data has for values.
#foreach my $field ( sort keys %{ $data } ) {
#	print "$field: $data->{ $field }\n";
#}

if ($data->{'Slave_SQL_Running'} eq 'Yes') {
	$status = 'Replicating from '. $data->{'Master_Host'};
	$state = 'OK';
	print $state .': '. $status ."\n";
	exit($ERRORS{$state});
} elsif ($data->{'Slave_SQL_Running'} eq 'No') {
	...
	$status = 'Unknown slave status: (Running: '. $data->{'Slave_SQL_Running'} .')';
	$state = 'UNKNOWN';
	print $state .': '. $status ."\n";
	exit($ERRORS{$state});
}
