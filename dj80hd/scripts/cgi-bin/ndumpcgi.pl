...
#
#--------------------------------------------------------------------------
# 11/23/2005 Add x.x support for gzip content                                 -
# 12/12/2005 Add x.x version number, chop to each line
# 02/28/2007 Add 0.3 IFRAME to display results
# 04/01/2007 Add 0.4 Support for gzip content WITHOUT embedded headers.
# 09/28/2007 Add 0.5 Fix gzip permission problem (NOT FIXED YET)
# 05/13/2008 Add 0.5 work_around_apache_bug                        
# 05/14/2008 Add 0.6 support for desktop view                      
#--------------------------------------------------------------------------
$VERSION = "0.6";
use CGI;
...
print "<script>function work_around_apache_bug() { var textarea = document.getElementById('text'); var current = textarea.value; var new_value = current.replace(/\\.\\./g,\"__\"); textarea.value = new_value; } </script>\n";
print "<h1 align=center>RFTRACE XML EXTRACTOR version $VERSION</h1><form method=post><textarea id=text cols=106 rows=32 name=data>";
...
else
{
	...
	{
		&printExampleCompressedContent();

	}
        elsif ($example eq "Show me example desktop content!")
	{
		&printExampleDesktopContent();
	}
	else #regular example
	{
		&printExampleUncompressedContent();
  	}
}

print "</textarea><br>\n";
print "<table border><tr><th><th>XML<th>Desktop<th>Other<tr><th>Functions<td>\n";
print "<input name=submit type=submit value='Extract and Clean XML !' onclick='work_around_apache_bug();'>";
print "<td><input name=submit type=submit value='Decode Desktop Content' onclick='work_around_apache_bug();'>";
print "<td><input type=submit name=reset value=Reset>\n";
print "<tr><th>Examples\n";
print "<td><input type=submit name=example value='Show me example uncompressed data !'>";
print "<br><input type=submit name=example value='Show me example compressed data !'>";
print "<td><input type=submit name=example value='Show me example desktop content!'>";
print "</table></form>";
...
# If there is data and the user actually requested a decode action...
if ($data && $submit)
{
   @lines = split(/\n/,$data);
   print "LINES=" . $#lines . "<br>";
   #print "<font color=blue>DATA:$data<br>SUBMIT:$submit</font>";
}
else
{
  print "NO DATA";
...
foreach (@lines)
{
   ...
   {
	   $out = $out . "<b>I DID NOT UNDERSTAND THIS LINE:</b> $_ <br>" unless (/0 \. \. \. \. 5/ || /^\s+$/);
   }#endif
...
#$allContent =~ s/\n//g;
#
if ($allContent =~ /Content-Location:\s+(\S+)/)
{
  $base = $1;
}

if (($allContent =~ /Content-Encoding:\s+gzip/) || ($allHexData =~ /^1F8B08/))
{
	$filename = &randomFileName("surveydata/tmp","jim.gz");
	


	#Cut of the headers and just get the content. If it is already gzip format, just
  #leave it alone
	$allHexData = ($allHexData =~ /^1F8B08/) ? $allHexData : &extractHexContentFromRFTRACE(@lines);
  ...
  if ($submit eq "Decode Desktop Content")
	{
      $result = &command2String("gzip -d $filename");
      $uncompressed_filename = substr ($filename,0,length($filename) -3);
      if (! -e $uncompressed_filename)
      {
         print "<font color=red>THIS FILE DOES NOT EXIST: $uncompressed_filename</font>"; exit 0;
      }
      
      $out = $out . &file2HexDumpString($uncompressed_filename);#remove .gz  

          
	}
	else #assume normal decode of gzip to xml
	{
...
sub printExampleCompressedContent
{
  print <<END_OF_AMAZON;
      . . . . 5 . . . . 0 . . . . 5 . . . . 0 . . . . 5 . . . . 0 . .
0000 436F6E74656E742D547970653A20746578742F786D6C0D0A436F6E74656E742D     Content-Type: text/xml..Content-
...
0019 5276F274DBF4317997A53C63649A883CE7D982C070E445CA2255888C47F2883C     Rv.t..1y..<cd__<....p.E."U..G..<
...
#
#
sub printExampleDesktopContent
{
  print <<END_OF_DESKTOP;
...
0002 7272612D506167652D49643A203134333036323930330D0A4E6F76617272612D     rra-Page-Id: 143062903..Novarra-
0003 5552493A20687474703A2F2F7777772E676F6F676C652E636F6D2F0D0A436F6E     URI: http://www.google.com/..Con
0004 74656E742D456E636F64696E673A20677A69700D0A4E6F76617272612D436F6E     tent-Encoding: gzip..Novarra-Con
0005 74656E742D4C656E6774683A20313236330D0A436F6E74656E742D4C656E6774     tent-Length: 1263..Content-Lengt
0006 683A203734320D0A0D0A1F8B08000000000000007593C14F134114C6DFDB4211     h: 742..............u..O.A....B.
0007 D9A2564800499C70100C2A0513A309D1E0412882456BC083079776D8DDB8DD69     ..VH.I.p..*......A(.Ek....v....i
0008 B62DC4C484281762A2097F84072E7A919888378D44E35D4F9E0C07130F5EE444     .-...(.b.....z...7.D.]O.....^.D
0009 F09BEE6EA34DE430BC99EFF7BDB7F3FA86984E11534B42181D9420832AD482F5     ...n.M.0..........N.SKB... .*...
0010 04B5D6D724569E54CAF6242598F0A7C146A0FF538A0E539ABAC8A4C4825C4CD1     ....\$V.T..\$%....F..S..S......\\L.
0011 10766DD8B1D0EA67A6113A8E434E2148233029992D59B6AC44EE7BCDBC0DBE57     .vm....g..:.CN!H#0).-Y..D.{....W
0012 F336945E282DB35639A69F34D31BA0FB34BD01A54FD337E44A4CBF6AA6B7418F     .6.^(-.V9..4....4...O.7.JL.j..A.
0013 6A7A1BCA289443794795CBAE6F478EDFCD8E7D3806B4631FCA0094D6C992E57A     jz..(.CyG...oG....}8..c........z
0014 21CECD97657DD97EE0AC2FDBAF3FA7A40299E2EB0D30C5B7E2F82D1B9BE047C0     !...e}.~../..?.......0....-...G.
0015 23482330A9CD8DFA5D2F607CC571B7EEAD781895307EC232AC2D3FA10D6B4BDE     #H#0....]/`|.q...x..0~.2.-?..kK.
0016 B57DE1FA494ED2107791AFC1838303A6EFF49C1740D1DEDE9EDE6D616710ADAE     .}..IN..w...........\@.....mag...
0017 AED635C4E9183242C8B8141E13EDEEEE3277D36BA44B8716EC76B08BC8BAA613     ..5...2B........2w.k.K...v......
0018 445094971F85C7518304BDA33BF8C654782B9197565070EA6854620DFC7AA3C4     DP.....Q....;..Tx+..VPp.hTb..z..
0019 1A4AAC374AACC52542282A61B4FF53E2194A0428712C3B5812D7A4F4F06B8A99     .J.7J..%B(*a..S..J.(q,;X.....k..
0020 5AE1FE83A84CBD6BE3F4921675D7C6E90502938E4C14972DBF208BD13769649B     Z....L.k....u.......L..-. ..7id.
0021 A643E40D02933AE602B9240309AAA2E51DBA1BCA1F1098D43963F9760DA32C6E     .C....:...\$.............9c.v..,n
0022 2BE54533C797E91B5A68526F6E590662F4C2994C26230A8EEB159148605949F1     +.E3....ZhRonY.b...L&#.....H`YI.
0023 2CFDA0A760BA8B4A153D59114B2A106187062B98900073380EBDDD5125594685     ,...`..J.=Y.K*.a..+...s8...Q%YF.
0024 73D1786539419E1EAF2C1B084C3A3AAFAAB26EAF3A52283F9A1BDE827E5ABF35     s.xe9A...,..L::...n.:R(?....~Z.5
0025 9C5B55A49CE74E3C6E7469C5F53C6195CBB8B47070B938F5149FA7C73AF5148F     .[U...N<nti..<a....pp.8.....:...
0026 2130A90B0D9241D5ADE87ECE05CA0EAC527CC94F60C2B13C1B4F7E8EAF504DFB     !0....A...~.....R|.O`..<.O~..PM.
0027 7368410D5AFA6A0D5659A988BCF26A5557F9B1FB3D8826B783177253BB1D9E46     shA.Z.j.VY....jUW...=.&...rS...F
0028 609239B1A86A55F1F76360979728A7B5CDB14CE66243C3A8137DFCF22B01A827     `.9..jU..c`..(....L.bC...}..+..'
0029 71927AFE7764FE010E3A5AFDEF040000                                     q.z.wd...:Z.....                
END_OF_DESKTOP
}
...
#
#
sub printExampleUncompressedContent
{
  print <<END_OF_GOOGLE;
...
sub file2HexDumpString
{
  my $file = shift;
  my $buffer;
  my $address = 0;
  my $out = "";
  open (F,$file) || die "Could not open this file: " . $f;
  binmode F;
  while (read(F,$buffer,16))
  {
  $out .= sprintf( "%08x: %s | %-16s\n", $address, hexstr($buffer),
      makesafe($buffer) );
    $address += 16;
