...
use CGI;
#----------------------- TO BE CONFIGURED ------------------------------
#$MY_URL = "http://127.0.0.1:8888/cgi-bin/survey2.pl";
...
sub viewResults
{
	...
	print "<b>The following people have not yet responded:</b><br>";
...
sub createSurvey
{
  $q = shift;
	...
	$name = &nameToSurveyId($origname);
	$linesForFile .= "SURVEY_NAME|" . $name . "\n";
	...
	{
		$ids_for_email{$_} = &randomPassword;
	}
	foreach (keys(%ids_for_email))
	{
		$linesForFile .= "SURVEY_INVITED|" . $_ . "|" . $ids_for_email{$_} . "\n";
	}
	...
	&writeSurveyFile($name,$linesForFile);	
...
sub sendEmails #($message,@emails)
{
   my $messsage = shift;
	...
	{
		$msgurl = $MY_URL . "?code=" . $ids_for_email{$email} . "&name=" . $name . "&action=Take%20Survey%20Form";
		$msga = "<a href='" . $msgurl . "'>" . $msgurl . "</a>";		
		$msg = $invite = $q->param('invite');
		$msg .= "\n\n" . $message . $msgurl;
