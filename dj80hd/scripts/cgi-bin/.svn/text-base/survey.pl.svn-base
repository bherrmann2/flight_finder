...
# 07/25/2005 - fix bug where link after creating survey has no name.
# 10/26/2005 - Checkbox interface for Multiple Selects   
# 11/09/2005 - Add "From" when sending reminder from deadbeats     
# 11/10/2005 - on multi-choice show those that voted yes vs. no   
# 11/10/2005 - on multi-choice show those that voted CHECKED vs. UNCHECKED  
# 11/10/2005 - only show CHECKED vs. UNCHECKED   for type MULTI_SELECT    
# 11/16/2005 - Add ballots to Admin form. 
# 11/16/2005 - Offer Vote Change
#
# TODO
...
# - Preview Survey before Committing.
...
# - need a function to escape Urls for display in HTML (e.g. &amp; not &)
#
# FEATURES
...
elsif ($action eq "Change Vote Form")
{
  &loadData($surveyName);
  &takeSurveyAgain();
}
elsif ($action eq "Change Vote")
{
  &loadData($surveyName);
  &changeVote;
}
...
{ 
  ...
  close F;
	...
	#Clear out the dups for anyone that changed their vote (both votes are in
	#the file.
	#SURVEY_ANSWER|1|4nl0Kpff
	foreach (@lines)
	{
	  if (/(^SURVEY_ANSWER\|\d+\|\S+\|)/)
		{
		  #overwrite any vote that was re-cast.
		  $answers{$1} = $_;
		}
		else
		{
	...
	return @returnLines, values(%answers);
}#getLines
...
sub adminSurveyForm
{
	...
	print "</form>";
	print "<hr>";
	print &ballotListAsHtml;
}

sub ballotListAsHtml
{
   $ballotList = "";
	 foreach (@NOSHOW_EMAILS)
	 {
	    $ballotList .= "<font color=red><b>$_ :</b><a href='" . &getBallotUrlFromEmail($_) . "'> VOTE </a></font><br/>";
	 }
	 	 foreach (@RESPONDANT_EMAILS)
	 {
	    $ballotList .= "<font color=green><b>$_ :</b><a href='" . &getBallotUrlFromEmail($_) . "'> VOTE </a></font><br/>";
	 }
	 
	 return $ballotList;
}
sub getBallotUrlFromCodeAndName #($code,$surveyName)
{
my $code = shift;
my $surveyName = shift;
return $MY_URL . "?code=" . $code . "&name=" . $surveyName . "&action=Take%20Survey%20Form";
}
sub getBallotUrlFromEmail
{
   my $email = shift;
	 	return  &getBallotUrlFromCodeAndName(&emailToId($email),$surveyName);
		
}

sub getBallotHtmlLinkFromEmail #($email)
{
   my $email = shift;
   return "<a href='" . &getBallotUrlFromEmail($email) . "'>" . &getBallotUrlFromEmail($email) . "</a>";
}
...
sub parseInvited
{
	...
	foreach (@lines)
	{
		...
		elsif (/^SURVEY_ADMIN_EMAIL/)
		{
			...
			@values = split /\|/,$_;
			$dummy = shift @values;
			$adminEmail = shift @values;
			if ($adminEmail) {$FROM_EMAIL = $adminEmail;}
		}	
	}
...
sub getQuestionHtml
{
  ...
  my $index = shift;
	...
	{
		#---FIMXE--- Choices cannot contain single quotes: '
		my $multipleChoiceCounter = 0;
		$string .= "<br/><i><small>(please check one or more of the following)</small></i><br/>";
		foreach (@{$hash_ref->{"CHOICES"}})
		{
			$string .= "<input type=checkbox name='";
			$string .= "answer-" . $index . "-";
			$string .= $multipleChoiceCounter;    
			$string .= "' value='" . $_ . "'>";   
			$string .= $_ . "<br>";               
...
sub takeSurveyAgain #()
{
   ...
   &takeSurvey(1); #change vote
}
sub takeSurvey #($changeVote[1,0])
{
  my $changeVote = shift;
	...
	print &getEmail($code) . "</b><br>";
	if (! &isInvited($code))
	{
		print "<b>You have not been invited to this survey !</b>";
		return;
	}
	if (&hasAlreadyVoted($code) && (!$changeVote))
	{
		...
		print "<br>To change your vote go here:";
		print "<a href='" . &getChangeVoteFormUrl . "'>" . &getChangeVoteFormUrl . "</a>";
		return;
	}
	...
	if ($changeVote)
	{
	$html .= "<input type=submit name=action value='Change Vote'>";
	}
	else
	{
	...
	$html .= "<input type=hidden name=name value=\"$surveyName\">";
...
sub getThankYouMessage
{
return "Thank you for voting.  To see the results click here:" . &getViewResultsHtml;
}
sub isInvited
{
	$code = shift;
...
sub getVoteLines
{
	...
	foreach ($q->param())
	{
		...
		if (/answer-(\d{1,2})/)
		{
			...
			#We assume that this works - normally this is seen in scalar context
	...
	foreach $question_hashref (@{$QUESTIONS})
	{
		...
		else
		{
			...
			foreach $key (keys (%{$tally_hashref}))
			{
				...
				#Subtract the array of those that voted for this item and subtract
				#it from the list of those that have voted to get the list of 
				#people that DID NOT vote for this item.
				
				@thoseThatDidNotVoteForThisItem = &inAnotB(\@RESPONDANT_EMAILS,
																				\@voterlist);
				
				if ($question_hashref->{"TYPE"} eq "MULTIPLE_SELECT")
				{
				#Those emails that did vote for this item.
				print "<td><font color=green><b>CHECKED:</b> ", join(", ",
					@voterlist), "</font>";
				#Those emails that did NOT vote for this item.
				print " <br><font color=red><b>UNCHECKED:</b> ", join(", ",
					@thoseThatDidNotVoteForThisItem), "</font>";
				}
				else
				{
				print "<td><font color=green><b></b> ", join(", ",
					@voterlist), "</font>";
				}
...
#returns an array of the elements in array A, not in array B
#NOTE: references to the arrays are passed in, not the arrays themselves.
sub inAnotB #($aArrayRef, $bArrayRef)
{
	$aArrayRef = shift;
	$bArrayRef = shift;
	...
	foreach $item(@{$bArrayRef})
	{
		$seen{$item} = 1;
	}
	foreach $item(@{$aArrayRef})
	{
		unless ($seen{$item})
		{
			push (@aOnly,$item);
...
sub getChangeVoteFormUrl
{
 return $MY_URL . "?code=" . $code . "&name=" . $surveyName . "&action=Change+Vote+Form";
}
sub getViewResultsUrl
{
	$surveyName = $q->param('name') unless $surveyName;
	$MY_URL . "?name=" . $surveyName . "&action=View+Results";
}
