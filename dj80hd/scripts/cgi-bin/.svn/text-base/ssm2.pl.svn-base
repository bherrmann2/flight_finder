...
$SENDMAIL = '/usr/lib/sendmail -oi -t -f jim@scissorsoft.com';
$VERSION = 11;
print $CONTENT_TYPE;

if ($q->param("to"))
{
  if (($q->param("to") =~ /novarra/i) || ($q->param("to") =~ /werwath/) || ($q->param("to") =~ /jason/i) || ($q->param("to") =~ /wisti/i) || ($q->param("to") =~ /scissorsoft/i))
	{
	#were ok
	}
	else
...
{
print "<form method=post>To:<input name=to value=werwath\@hotmail.com><br>Subject:<input name=subject value=foo><br>Message:<input name=msg value=bar><input type=submit value=Send></form>";
}
